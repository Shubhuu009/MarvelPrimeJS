...
module.exports = {
    name: 'vcunmuteall',
    aliases: ['voice-unmuteall', 'unmutevc-all'],
    category: 'utility',
    ...
    data: new SlashCommandBuilder()
        .setName('vcunmuteall')
        .setDescription('Unmutes everyone in your current voice channel.')
    ...
    async execute(context) {
        ...
        if (!moderator.permissions.has(PermissionFlagsBits.MuteMembers)) {
            const noPerms = result(user, 'Voice Result', 'VC Unmute All', 'You do not have permission.', false, 'Missing Mute Members permission.');
        ...
        if (!moderator.voice?.channel) {
            const err = result(user, 'Voice Result', 'VC Unmute All', 'You must be in a VC to unmute someone.', false, 'Not connected to voice.');
        ...
        try {
            for (const [id, m] of members) {
                // Skip the bot and those not server muted
                if (m.id === client.user.id || !m.voice.serverMute) continue;
                
                // Native Discord Audit Log: Reason is passed here
                await m.voice.setMute(false, `VC Unmuteall by ${user.username}`).catch(() => null);
                count++;
            }
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(0x00f53d)
                .setDescription(
                    `<:Marvel_Successfully:1417856966352568472> **Action :** VC Unmute All\n` +
            ...
            const fail = result(user, 'Voice Result', 'VC Unmute All', 'A fatal error occurred during the mass unmute operation.', false, 'Internal error.');
