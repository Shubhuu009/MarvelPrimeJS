...
module.exports = {
    name: 'hide',
    description: "Hides a channel from the @everyone role.",
    category: 'moderation',
    ...
    data: new SlashCommandBuilder()
        .setName('hide')
        .setDescription("Hides a channel from the @everyone role.")
        .addChannelOption(opt => 
            opt.setName('channel')
                .setDescription('The channel to hide (default: current channel)'))
        .addStringOption(opt => 
            opt.setName('reason')
    ...
    async execute(context, args) {
        ...
        // Logic check: Is it already hidden?
        ...
        if (currentPermissions && !currentPermissions.has(PermissionFlagsBits.ViewChannel)) {
            return this.failure(context, targetChannel, "This channel is already hidden! 👻", isSlash);
        }

        try {
            // Action: Set ViewChannel to false for @everyone
            await targetChannel.permissionOverwrites.edit(everyoneRole, {
                ViewChannel: false
            }, { reason: `Hidden by ${author.username} | ${reason}` });

            // Log to Audit Log
            const auditLog = await guild.fetchAuditLogs({ type: 111, limit: 1 }).catch(() => null);

            const successEmbed = new EmbedBuilder()
                .setTitle("**Channel Hide Result**")
                .setColor(0x00f53d)
                .setDescription(
                    `**__Action : Hide__**\n` +
                    `<:Marvel_moderator:1417818626769027184> **Moderator :** \`${author.username}\`\n` +
                    `<:Marvel_Successfully:1417856966352568472> **Successful hidden**\n` +
            ...
            logger.error(`Hide Command Error: ${err.message}`);
            return this.failure(context, targetChannel, "An unexpected error occurred while hiding the channel.", isSlash);
        }
    },
    ...
    failure(context, channel, reason, isSlash) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(
                `**__Action : Hide__**\n` +
                `<:Marvel_moderator:1417818626769027184> **Moderator :** \`${author.username}\`\n` +
                `<:marvel_Cross:1417857962688512203> **Unsuccessful hide**\n` +
    ...
    sendHelp(context) {
        ...
        const embed = new EmbedBuilder()
            .setTitle("⚙️ How to use the Hide Command")
            .setColor(0x0099ff)
            .setDescription(
                ` \n \`\`\`\n${prefix}hide [#channel] [reason]\n\`\`\` \n \`\`\`\n${prefix}hide 123456789 [reason]\n\`\`\``
            );
        if (!isSlash) {
