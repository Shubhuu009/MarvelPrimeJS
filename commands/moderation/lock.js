...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('lock')
        .setDescription("Locks a channel to prevent members from sending messages.")
    ...
    async execute(context, args) {
        ...
        // Logic check: Is it already locked?
        ...
        if (currentPermissions && !currentPermissions.has(PermissionFlagsBits.SendMessages)) {
            return this.failure(context, targetChannel, "This channel is already locked! 🔒", isSlash);
        }

        try {
            // Action: Disable SendMessages for @everyone
            await targetChannel.permissionOverwrites.edit(everyoneRole, {
                SendMessages: false
            }, { reason: `Locked by ${author.username} | ${reason}` });

            // Log to Audit Log
            const auditLog = await guild.fetchAuditLogs({ type: 111, limit: 1 }).catch(() => null);

            const successEmbed = new EmbedBuilder()
                .setTitle("**Channel Lock Result**")
                .setColor(0x00f53d)
                .setDescription(
                    `**__Action : Lock__**\n` +
                    `<:Marvel_moderator:1417818626769027184> **Moderator :** \`${author.username}\`\n` +
                    `<:Marvel_Successfully:1417856966352568472> **Successful lock**\n` +
            ...
            logger.error(`Lock Command Error: ${err.message}`);
            return this.failure(context, targetChannel, "An unexpected error occurred while locking the channel.", isSlash);
        }
    },
    ...
    failure(context, channel, reason, isSlash) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(
                `**__Action : Lock__**\n` +
                `<:Marvel_moderator:1417818626769027184> **Moderator :** \`${author.username}\`\n` +
                `<:marvel_Cross:1417857962688512203> **Unsuccessful lock**\n` +
    ...
    sendHelp(context) {
        ...
        const embed = new EmbedBuilder()
            .setTitle("⚙️ How to use the Lock Command")
            .setColor(0x0099ff)
            .setDescription(
                ` \n \`\`\`\n${prefix}lock [#channel] [reason]\n\`\`\` \n \`\`\`\n${prefix}lock 123456789 [reason]\n\`\`\``
            );
        if (!isSlash) {
